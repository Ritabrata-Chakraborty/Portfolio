...
This work presents a generic computer vision and machine learning pipeline for sports trajectory analysis, with field hockey as a case study. We address the lack of comprehensive shot statistics and trajectory analysis in underserved sports domains. The pipeline features: (1) 3D trajectory reconstruction using PnLCalib calibration with extensive detector ablations (YOLO, RF-DETR, BlurBall, WASB) and Bayesian optimization for parametric trajectory fitting, achieving sub-meter localization accuracy; (2) comprehensive shot classification evaluating temporal models (TCN, LSTM, BiLSTM, GRU, Transformer) and classical ML methods (XGBoost, Random Forest, SVM, KNN) across seven field hockey shot types, achieving 96.4% accuracy with TCN; (3) a public dataset extracted from broadcast videos with annotated 3D trajectories and shot labels. The modular pipeline is reusable across different sports and provides a framework for advancing sports analytics and automated coaching systems.
